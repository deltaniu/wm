...
let addTask = async () => {
  ...
  let chapters = [];
  for (
    let cptEle = cptEles.firstElementChild, cptIdx = 1;
    cptEle != undefined;
    ...
    for (
      let subCptEle = subCpts.firstElementChild, subCptIdx = 1;
      subCptEle != undefined;
...
let getSource = async () => {
  let src = document.querySelector(
    "#root > div.course__course-page--3J8Sy > div.course__video-row--20SaE > div:nth-child(1) > div.course__player-container--1S_cH > div > div.VideoWrapper__video-wrapper--3FBpC > video > source"
  );
  if (src) {
